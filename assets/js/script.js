(function(){
  ...
  window.clearSocialImage=function(){
    const input=document.getElementById('postImageInput'); if(input) input.value='';
    const img=document.getElementById('postImagePreview'); if(img){ img.removeAttribute('src'); }
    document.getElementById('postImagePreviewWrap')?.classList.remove('show');
    const name=document.getElementById('postImageFileName'); if(name) name.textContent='JPG, PNG veya WEBP. Maksimum 2 MB.';
  };
